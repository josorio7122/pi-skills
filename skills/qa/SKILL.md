---
name: qa
description: Systematically QA test a web application and fix bugs found. Runs QA testing, then iteratively fixes bugs in source code, committing each fix atomically and re-verifying. Use when asked to "qa", "QA", "test this site", "find bugs", "test and fix", or "fix what's broken". Also use when the user says a feature is ready for testing or asks "does this work?". Three tiers: Quick (critical/high only), Standard (+ medium), Exhaustive (+ cosmetic). Produces before/after health scores, fix evidence, and a ship-readiness summary. For report-only mode, use the qa-only skill.
---
...
| Mode | full | `--regression .pi/qa-reports/baseline.json` |
| Output dir | `.pi/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .pi/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `.pi/reports/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
...
3. **Detect the running app** — check common local dev ports:
   ```bash
   playwright-cli goto http://localhost:3000 2>/dev/null && echo "Found app on :3000" || \
   playwright-cli goto http://localhost:4000 2>/dev/null && echo "Found app on :4000" || \
   playwright-cli goto http://localhost:8080 2>/dev/null && echo "Found app on :8080"
   ```
...
11. **Show screenshots to the user.** After every `playwright-cli screenshot`, `playwright-cli snapshot` + `playwright-cli screenshot`, or `playwright-cli resize 375 812
playwright-cli screenshot` command, use the Read tool on the output file(s) so the user can see them inline. For `responsive` (3 files), Read all three. This is critical — without it, screenshots are invisible to the user.
...
playwright-cli screenshot --filename="$REPORT_DIR/screenshots/issue-NNN-after.png"
playwright-cli console error
playwright-cli snapshot
```
...
- Include full attribution comment:
  ...
  // Found by the QA workflow on {YYYY-MM-DD}
  // Report: .pi/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.pi/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
# Project slug — use project directory name
```
Write to `.pi/reports/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by the QA workflow on {branch}, {date}"
