...
**Local:** `.pi/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
# Project slug — use project directory name
```
Write to: `.pi/reports/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
.pi/reports/designs/design-audit-{YYYYMMDD}/
...
# Project slug — use project directory name
```
Write a one-line summary to `.pi/reports/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
