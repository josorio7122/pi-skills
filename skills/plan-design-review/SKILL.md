...
### The design mockup tool — YOUR PRIMARY TOOL

You have the **design mockup tool**, an AI mockup generator that creates real visual mockups
...
- If no DESIGN.md: "No design system found. Recommend creating a DESIGN.md first (use the design-consultation skill). Proceeding with universal design principles."
...
design mockup tool is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the design mockup tool. This is how we
...
`.pi/reports/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The design mockup tool
...
- `mkdir -p .pi/reports/designs/...`
- `# Design mockup generation not available in pi-agents`, `# Design variant generation not available in pi-agents`, `# Design comparison not available in pi-agents`, `# Design iterate not available in pi-agents`, `# Design tool not available: evolve`, `# Design check not available in pi-agents`
...
fewer variants and benefits from sequential control. Note: the design shotgun skill uses
...
```bash
# Design variant generation not available in pi-agents --brief "<description assembled from plan + DESIGN.md constraints>" --count 3 --output-dir "$_DESIGN_DIR/"
```
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The design mockup tool isn't set up yet. Run `# Design tool not available: setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
```bash
# Design mockup generation not available in pi-agents --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/pi-agents-ideal-<dimension>.png
```
...
FIX TO 10: If DESIGN.md exists, annotate with specific tokens/components. If no DESIGN.md, flag the gap and recommend creating a DESIGN.md first.
...
If all passes 8+: "Plan is design-complete. Run the design review skill after implementation for visual QA."
...
|----------------|-------------|-----------|-------|
| [screen name]  | .pi/reports/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
