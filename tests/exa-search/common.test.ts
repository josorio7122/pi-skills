...
describe('requireArg', () => {
  it('returns value when provided', () => {
    expect(requireArg({ value: 'hello', name: 'test' })).toBe('hello')
  })

  it('exits with code 1 when value is undefined', () => {
    const result = runInline(`import { requireArg } from '${COMMON}'; requireArg({value: undefined, name: 'testarg'});`)
    expect(result.status).toBe(1)
  })

  it('prints error message to stderr when value is undefined', () => {
    const result = runInline(`import { requireArg } from '${COMMON}'; requireArg({value: undefined, name: 'myarg'});`)
...
describe('filterOptions', () => {
  it('returns only the keys listed in the keys array', () => {
    expect(filterOptions({ opts: { a: 1, b: 2, c: 3 }, keys: ['a', 'c'] })).toEqual({ a: 1, c: 3 })
  })

  it('omits keys whose value is undefined', () => {
    const result = filterOptions({ opts: { a: 1, b: undefined } as Record<string, unknown>, keys: ['a', 'b'] })
    expect(result).toEqual({ a: 1 })
    expect('b' in result).toBe(false)
  })

  it('keeps keys whose value is null or 0', () => {
    const result = filterOptions({ opts: { a: null, b: 0, c: false }, keys: ['a', 'b', 'c'] })
  ...
  it('returns {} when no keys match', () => {
    expect(filterOptions({ opts: { x: 1 }, keys: ['a', 'b'] })).toEqual({})
  })
})
...
describe('handleError', () => {
  it('exits with code 1', () => {
    const result = runInline(`import { handleError } from '${COMMON}'; handleError(new Error('boom'));`)
    expect(result.status).toBe(1)
  })

  it('prints error message to stderr', () => {
    const result = runInline(`import { handleError } from '${COMMON}'; handleError(new Error('test-error-message'));`)
