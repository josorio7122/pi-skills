...
import {
	createClient,
	PostHogError,
} from '../../skills/posthog-skill/scripts/lib/posthog-client.js'
...
describe('posthog-client: Authorization header', () => {
	it('sets Authorization: Bearer <token> on GET requests', async () => {
		...
		expect((fetch.calls[0]!.options.headers as Record<string, string>).Authorization).toBe(
			`Bearer ${CONFIG.token}`,
		)
	})
...
describe('posthog-client: Content-Type header', () => {
	it('sets Content-Type: application/json on POST requests', async () => {
		...
		const client = createClient(CONFIG, fetch)
		await client.createDashboard('Test', [])
		expect((fetch.calls[0]!.options.headers as Record<string, string>)['Content-Type']).toBe(
			'application/json',
		)
	})
})
