## builder: ## Create curated Render API reference and update skill docs
...
- `skills/render-deploy/references/render-api-spec.json` — **deleted** (raw 6MB+ spec replaced by curated reference)
- `skills/render-deploy/references/render-api.md` — **created** (~120 lines), covering auth, PostgreSQL creation, Key-Value creation, connection string retrieval, metrics queries, and env var management via REST API
- `skills/render-deploy/SKILL.md` — **3 targeted edits**:
  1. Updated `compatibility` frontmatter to mention `RENDER_API_KEY`
  2. Added note after auth step pointing to `render-api.md` for database/metrics
  3. Replaced "Visit Dashboard for metrics" with REST API reference
- `skills/render-deploy/references/direct-creation.md` — **3 targeted edits**:
  1. Replaced Postgres "Not available via CLI" Dashboard link → `render-api.md` reference
  2. Replaced Key-Value "Not available via CLI" Dashboard link → `render-api.md` reference
  3. Replaced metrics "Visit Dashboard" note → REST API reference

**Verification:**
- `render-api-spec.json` does not exist ✓
- `render-api.md` exists and is well-formatted ✓
- No remaining "Visit Dashboard" fallbacks for things the API can do ✓
